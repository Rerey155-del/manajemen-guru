...
export const useClassStore = defineStore('class', {
  ...
  actions: {
    ...
    async createItem(payload: Omit<ClassType, 'id'>) {
      this.error = null;
      try {
    ...
    async updateItem(id: number | string, payload: Partial<ClassType>) {
      this.error = null;
      try {
    ...
    async toggleItemStatus(id: number | string, statusField: string, newValue: any) {
      this.error = null;
      try {
        await classService.updateStatus(id, statusField, newValue);
        const index = this.items.findIndex(i => i.id === id);
        if (index !== -1) {
